...
import {
  chapterLessonPreparationAiReviseTeachingContentService,
  chapterLessonPreparationGetTeachingContentByIdService,
  chapterLessonPreparationSaveTeachingContentService
...
const fetchContent = async () => {
  //查找数据库
  const res1 = await chapterLessonPreparationGetTeachingContentByIdService(chapterLessonPreparationId)
  if(res1.msg !== null){
  ...
  if(!markdownContent.value){
    ...
    try {
      const eventSource = new EventSource(
        'http://localhost:8080/chapterLessonPreparation/ai/teachingContent/2'
      )
...
const sendMessage = async () => {
  ...
  try {
    // 调用AI接口
    const response = await chapterLessonPreparationAiReviseTeachingContentService(markdownContent.value, userMessage)
...
const saveContent = async () => {
  ...
  isSaving.value = true
  try {
    await chapterLessonPreparationSaveTeachingContentService(
      2, // 这里使用实际的ID
