...
import { chapterLessonPreparationAiGenerateTeachingActivitiesArrangementService,
  chapterLessonPreparationGetTeachingActivitiesByIdService,
  chapterLessonPreparationSaveTeachingActivitiesService,
  chapterLessonPreparationAiReviseTeachingActivitiesService } from '@/api/chapterLessonPreparation.js'
import { h } from 'vue'
...
const sendMessage = async () => {
  ...
  try {
    ...
    // 这里添加与AI交互的逻辑
    const aiResponse = await chapterLessonPreparationAiReviseTeachingActivitiesService(teachingActivities.value, userInput.value)
...
onMounted(async () => {
  initChat()
  const res = await chapterLessonPreparationGetTeachingActivitiesByIdService(chapterLessonPreparationId)
  ...
  if(teachingActivities.value.length === 0) {
    ...
    try {
      const res = await chapterLessonPreparationAiGenerateTeachingActivitiesArrangementService(chapterLessonPreparationId)
      teachingActivities.value = res.data
...
const saveActivities = async () => {
  try {
    ...
    await chapterLessonPreparationSaveTeachingActivitiesService(teachingActivities.value,chapterLessonPreparationId)
